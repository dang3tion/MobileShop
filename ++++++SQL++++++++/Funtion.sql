...
USE TESTAAA
drop database testaaa
CREATE TABLE TIMEA(NGAY smallDATETIME NOT NULL PRIMARY KEY)



INSERT INTO TIMEA VALUES('2021/01/27 08:41:18')
...
RETURNS TABLE
RETURN		
SELECT SP.MASP,SP.TENSP,GIA.GIA,GIA.GIA_KM,HA.ANH,DG.AVGDANHGIA,SP.NGAYCAPNHAT,SP.LUOTXEM,SP.LOAI_SP,TH.TENTH,MS.TENMAU
FROM SANPHAM SP JOIN (
	SELECT G1.*
	FROM GIA_SP G1 
	JOIN (SELECT MASP, MAX(NGAYCAPNHAT) NGAYCAPNHAT
	 ...
	 JOIN(SELECT HA.*FROM HINHANH HA
						 WHERE HA.LOAIANH='NEN' AND MAMAU=@MAMAU)HA  ON HA.MASP=SP.MASP JOIN MAUSAC MS ON MS.MAMAU=HA.MAMAU
...
SELECT HA.MASP,HA.MAMAU,MS.TENMAU FROM HINHANH HA JOIN MAUSAC MS ON MS.MAMAU=HA.MAMAU WHERE HA.MASP='sp05' GROUP BY HA.MASP,HA.MAMAU,MS.TENMAU
